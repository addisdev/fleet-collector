...
export function registerSystem(app: FastifyInstance) {
  ...
  app.get("/api/artifacts", async (req) => {
    ...
    if (q.q) {
      where.push("(name LIKE ? OR sha256 LIKE ?)");
      params.push(`%${q.q}%`, `${q.q}%`);
    }
    const sql = where.length ? ` WHERE ${where.join(" AND ")}` : "";
    const total = (db.prepare(`SELECT COUNT(*) AS n FROM artifacts${sql}`).get(...params) as { n: number }).n;
    const rows = db
      .prepare(`SELECT sha256, name, size, created_at FROM artifacts${sql} ORDER BY created_at DESC LIMIT ? OFFSET ?`)
      .all(...params, per_page, offset) as { sha256: string; name: string | null; size: number; created_at: string }[];
